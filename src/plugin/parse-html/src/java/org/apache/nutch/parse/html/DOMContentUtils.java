...
public class DOMContentUtils {
  ...
  public boolean getText(StringBuffer sb, Node node,
      boolean abortOnNestedAnchors, URL baseUrl, JsonArray links) {
    if (getTextHelper(sb, node, abortOnNestedAnchors, 0, baseUrl, links)) {
      return true;
    }
    return false;
  }
  ...
  public boolean getText(StringBuffer sb, Node node, boolean abortOnNestedAnchors) {
    return getText(sb, node, false, null, null);
  }
  ...
  private boolean getTextHelper(StringBuffer sb, Node node,
      boolean abortOnNestedAnchors, int anchorDepth, URL baseUrl, JsonArray links) {
    boolean abort = false;
    ...
    while (walker.hasNext()) {
      ...
      if (links != null && "a".equalsIgnoreCase(nodeName)) {
        Element e = (Element)currentNode;
        String href = StringUtils.trim(e.getAttribute("href"));
        if (StringUtils.isNotEmpty(href)) {
          try {
            URI resultUri = baseUrl.toURI().resolve(URIUtil.encodeQuery(href));
            String protocol = resultUri.getScheme();
            if ("http".equalsIgnoreCase(protocol) || "https".equalsIgnoreCase(protocol)) {
                  JsonObject jsonObject = new JsonObject();
                  jsonObject.addProperty("url", URIUtil.decode(resultUri.toASCIIString()));
                  jsonObject.addProperty("offset", sb.length());

                  links.add(jsonObject);
            }
          } catch (URISyntaxException | URIException e1) {
            System.err.println("FAILED PARSED URL " + href);
          }
        }
      }
      if (nodeType == Node.TEXT_NODE) {
        // cleanup and trim the value
        cleanAndAddText(sb, currentNode.getNodeValue());
      }
    }
  ...
  private void cleanAndAddText(StringBuffer sb, String text) {
    if (text != null) {
