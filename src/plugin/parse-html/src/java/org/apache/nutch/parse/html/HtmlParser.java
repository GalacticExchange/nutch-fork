...
public class HtmlParser implements Parser {
  ...
  public ParseResult getParse(Content content) {
    ...
    if (!metaTags.getNoIndex()) { // okay to index
      ...
      if (conf.getBoolean("parser.html.extract.article", false)) {
        String tempHtml;
        try {
          tempHtml = new String(content.getContent(), encoding);
        } catch (UnsupportedEncodingException e) {
          LOG.error("Unsupported encoding " + encoding);
          tempHtml = new String(content.getContent());
        }

        try {
          text = ArticleExtractor.getInstance().getText(getBodyFromHtml(tempHtml));
        } catch (BoilerpipeProcessingException | TransformerException | IOException | SAXException e) {
          LOG.warn("Failed to get article from HTML: " + e.getMessage(), e);
        }
      }
      ...
      if (StringUtils.isEmpty(text)) {
        ...
        utils.getText(sb, root, false, base, links); // extract text
        metadata.set(Metadata.LINKS_WITH_OFFSET_ON_PAGE, links.toString());
        text = sb.toString();
        sb.setLength(0);
      }
  ...
  private String getBodyFromHtml(String text) throws TransformerException, IOException, SAXException {
